...
export default function ConnectionStatus({ state }: { state: ConnState }) {
  const config = {
    connected: { color: 'bg-success-500', label: 'Live' },
    connecting: { color: 'bg-text-secondary/40', label: 'Connecting...' },
    reconnecting: { color: 'bg-accent-warm', label: 'Reconnecting...' },
  }[state];

  return (
    <div className="inline-flex items-center gap-1.5 text-[11px] font-medium text-text-secondary">
